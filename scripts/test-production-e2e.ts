...
async function testCreditBalance(email: string) {
  console.log('\n💰 Test 3: Vérification solde crédits')
  console.log('='.repeat(50))

  try {
    const response = await fetch(`${API_BASE}/api/test/check-balance?email=${encodeURIComponent(email)}`)
    const data = await response.json()
    console.log('Réponse:', JSON.stringify(data, null, 2))

    if (response.ok) {
      console.log(`✅ Solde actuel: ${data.balance} crédits`)
      if (data.balance === 3) {
        console.log('✅ Les 3 crédits gratuits ont bien été attribués')
        return { success: true, balance: data.balance }
      } else {
        console.log(`⚠️  Attendu: 3 crédits, Réel: ${data.balance} crédits`)
        return { success: false, error: `Solde incorrect: ${data.balance} au lieu de 3` }
      }
    } else {
      console.log('❌ Impossible de récupérer le solde')
      return { success: false, error: data.error }
    }
  } catch (error) {
    console.log('❌ Erreur réseau:', error)
...
async function main() {
  ...
  if (registration.success) {
    ...
    // Test 4: Solde crédits
    results.creditBalance = await testCreditBalance(registration.email)
  }
  ...
  console.log(`Solde crédits: ${results.creditBalance?.success ? '✅' : '❌'}`)

  const allPassed = results.packs.success && results.registration.success && results.login?.success && results.creditBalance?.success
