...
import { Hand, FileText, Calculator, ArrowRight } from 'lucide-react'
  ...
  return (
    <div>
      ...
      <div className="mb-6 md:mb-8">
        <p className="text-sm capitalize text-secondary-500">{today}</p>
        <div className="mt-1 flex items-center gap-2">
          <h1 className="text-2xl font-bold tracking-tight text-secondary-900 md:text-3xl">
            Bonjour, {firstName}
          </h1>
          <Hand className="h-8 w-8 text-secondary-400" />
        </div>
        <p className="mt-1 text-secondary-600">Que voulez-vous faire aujourd’hui ?</p>
      </div>
      ...
      <div className="mb-6 sm:hidden">
        ...
        >
          ...
          <span className="btn btn-sm bg-white text-primary-700 hover:bg-primary-50">
            Acheter
          </span>
        </Link>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <Link
          href="/calculateur"
          className="card card-pad group flex items-center justify-between transition-all hover:shadow-md hover:shadow-secondary-900/5 active:scale-[0.99]"
        >
          <div>
            <p className="text-lg font-semibold text-secondary-900">Calculateur de prix</p>
            <p className="mt-0.5 text-sm text-secondary-600">Trouvez le bon prix de vente</p>
          </div>
          <span className="grid h-11 w-11 shrink-0 place-items-center rounded-full bg-primary-50 text-primary-600 transition-transform group-hover:translate-x-1">
            <ArrowRight className="h-5 w-5" />
          </span>
        </Link>
      </div>
      ...
      <div className="mt-6 grid grid-cols-3 gap-3 md:mt-8 md:gap-4">
        <div className="card p-4 md:p-6">
          <p className="text-xs font-medium text-secondary-500 md:text-sm">Factures créées</p>
          <p className="mt-1 text-xl font-bold text-secondary-900 md:text-3xl">
            {stats.totalInvoices}
          </p>
        </div>
        <div className="card p-4 md:p-6">
          <p className="text-xs font-medium text-secondary-500 md:text-sm">Total facturé</p>
          <p className="mt-1 text-xl font-bold text-secondary-900 md:text-3xl">
            {stats.totalBilled.toLocaleString('fr-FR')}
            <span className="ml-1 text-xs font-semibold text-secondary-500 md:text-sm">FCFA</span>
          </p>
        </div>
        <div className="card p-4 md:p-6">
          <p className="text-xs font-medium text-secondary-500 md:text-sm">Total payé</p>
          <p className="mt-1 text-xl font-bold text-accent-600 md:text-3xl">
      ...
      <div className="card mt-6 md:mt-8">
        <div className="flex items-center justify-between border-b border-secondary-200/80 px-5 py-4">
          <h2 className="font-semibold text-secondary-900">Dernières factures</h2>
          <Link
            href="/invoices"
        ...
        {user.invoices.length === 0 ? (
          <div className="flex flex-col items-center px-6 py-12 text-center">
            <span className="grid h-14 w-14 place-items-center rounded-2xl bg-secondary-100 text-secondary-400">
              <FileText className="h-7 w-7" />
            </span>
            <p className="mt-4 font-medium text-secondary-900">Aucune facture pour le moment</p>
              ...
              <li key={invoice.id}>
                ...
                >
                  <div className="min-w-0">
                    ...
                    <p className="mt-0.5 truncate text-sm text-secondary-500">
                      {invoice.clientName} · {new Date(invoice.createdAt).toLocaleDateString('fr-FR')}
                    </p>
                  </div>
                  <p className="shrink-0 font-semibold text-secondary-900">
                    {invoice.totalAmount.toLocaleString('fr-FR')}{' '}
                    <span className="text-xs font-medium text-secondary-500">FCFA</span>
                  </p>
                </Link>
              </li>
            ))}
