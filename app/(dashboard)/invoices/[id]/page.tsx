...
export default function InvoiceDetailPage() {
  ...
  const fetchInvoice = async () => {
    try {
      ...
      // Si la facture a déjà un PDF généré, on le marque
      if (data.shareUrl) {
    ...
    try {
      ...
      if (navigator.share && navigator.canShare({ files: [file] })) {
        await navigator.share({
          ...
          text: `Bonjour ${invoice.clientName}, voici votre facture n°${invoice.invoiceNumber} pour un montant de ${invoice.totalAmount.toLocaleString('fr-FR')} FCFA.`,
          files: [file],
        })
      } else {
        // Fallback : télécharger et laisser l'utilisateur partager manuellement
        setDownloadMessage({
          type: 'error',
          text: 'Partage non supporté sur ce navigateur. Le PDF a été téléchargé.',
        })
      }
    } catch (error) {
      ...
      <div className="mt-6">
        <div className="grid gap-3 sm:grid-cols-2">
          ...
          <a
            ...
            className={`btn btn-success btn-lg w-full ${!pdfGenerated ? 'opacity-50 cursor-not-allowed' : ''}`}
            onClick={(e) => {
              if (!pdfGenerated) {
                e.preventDefault()
                setDownloadMessage({
                  type: 'error',
                  text: 'Générez d\'abord le PDF pour envoyer sur WhatsApp.',
                })
              }
            }}
          >
        ...
        {!shareUrl && !downloadMessage && (
          <p className="mt-3 text-center text-xs text-stone-500">
            ...
            téléchargement (1 crédit). Une fois généré, vous pouvez le partager
            directement ou envoyer sur WhatsApp.
          </p>
        )}
      </div>
    </div>
